...
#Region "Delegate"
    ...
    Private Delegate Sub InvokeControlEnabledCallBack(ByVal status As Boolean, ByVal c As System.Windows.Forms.Control)
    ...
    Private Delegate Sub InvokeSelectedLineCallBack(ByVal Status As Boolean, ByVal c As CLineCtrl)
    ...
    Private Sub InvokeControlText(ByVal Message As String, ByVal c As System.Windows.Forms.Control)
        ...
        End Try
    End Sub
    Private Sub InvokeControlEnabled(ByVal status As Boolean, ByVal c As System.Windows.Forms.Control)
        Try
            If Me.InvokeRequired() Then
                '目前執行緒和UI執行緒不同...需使用Invoke對UI做更新
                Dim cb As New InvokeControlEnabledCallBack(AddressOf InvokeControlEnabled)
                Me.BeginInvoke(cb, status, c)
            Else
                '目前執行緒和UI執行緒相同...可以直接對UI做更新
                c.Enabled = status
    ...
    Private Sub InvokeSelectedLine(ByVal Status As Boolean, ByVal c As CLineCtrl)
        Try
            If Me.InvokeRequired() Then
                '目前執行緒和UI執行緒不同...需使用Invoke對UI做更新
                Dim cb As New InvokeSelectedLineCallBack(AddressOf InvokeSelectedLine)
                Me.BeginInvoke(cb, Status, c)
            Else
                '目前執行緒和UI執行緒相同...可以直接對UI做更新
                c.IsSelect = Status
...
#End Region
    ...
    Private Sub AddLog(ByVal strLog As String)
        m_Logs.Add(New Clog(strLog))
    End Sub
    Private Sub AddLog(ByVal strLog As String, ByVal newValue As String)
        m_Logs.Add(New Clog(strLog, newValue))
    End Sub
    Private Sub AddLog(ByVal paramName As String, ByVal oldValue As String, ByVal newValue As String)
        m_Logs.Add(New Clog(paramName, oldValue, newValue))
    End Sub
    ...
    Private Sub SetPermission()
        ...
        If (CLoginInformation.CurrentUser.Permission <= 0) Then
            ...
            If (TabControl1.TabPages.IndexOf(TabPageCorrection) > -1) Then
                TabControl1.TabPages.Remove(TabPageCorrection)
            End If
            'If (TabControl1.TabPages.IndexOf(TabPagePos) > -1) Then
            '    TabControl1.TabPages.Remove(TabPagePos)
            'End If
            ...
            AddLog("打開操作員畫面")
        Else
            AddLog(CChangeLanguage.GetString("打開" & CLoginInformation.CurrentUser.Username & "畫面"))
            btnAddLine.Enabled = True
            btnDelete.Enabled = True
            btnNewProject.Enabled = True
            btnEdit.Enabled = True
            btnSave.Enabled = True
            grbGraphicParameter.Visible = True
            grbGalvoParameter.Visible = True
            If (PropertyGrid1.SelectedObject IsNot Nothing) Then
            ...
            End If
            'If (TabControl1.TabPages.IndexOf(TabPagePos) = -1) Then
            '    TabControl1.TabPages.Insert(2, TabPagePos)
            'End If
            ...
            If (CLoginInformation.CurrentUser.Permission < 999) Then
                ...
                If (TabControl2.TabPages.IndexOf(TabPageCorrectionValue) > -1) Then
                    TabControl2.TabPages.Remove(TabPageCorrectionValue)
                End If
                If (TabControl1.TabPages.IndexOf(TabPageCorrection) > -1) Then
                    TabControl1.TabPages.Remove(TabPageCorrection)
                End If
            Else
                ...
                If (TabControl2.TabPages.IndexOf(TabPageCorrectionValue) = -1) Then
                    TabControl2.TabPages.Insert(1, TabPageCorrectionValue)
                End If
                If (TabControl2.TabPages.IndexOf(TabPageCalibration) = -1) Then
                    TabControl2.TabPages.Insert(2, TabPageCalibration)
                End If

                If (TabControl2.TabPages.IndexOf(TabPageAxis) = -1) Then
                    TabControl2.TabPages.Insert(4, TabPageAxis)
                End If
                If (TabControl2.TabPages.IndexOf(TabPageScale) = -1) Then
                    TabControl2.TabPages.Insert(5, TabPageScale)
                End If
            End If
        End If
    End Sub
    ...
    Private Sub FMain_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        End If
    End Sub
    Private m_AxisVisible As Boolean = True
    Private Sub DrawAxis(ByVal Visible As Boolean)
        Dim bmp As Bitmap = Nothing
        If (grbCamera.BackgroundImage IsNot Nothing) Then
            bmp = grbCamera.BackgroundImage.Clone()
            grbCamera.BackgroundImage.Dispose()
        Else
            bmp = New Bitmap(grbCamera.Width, grbCamera.Height)
        End If
        Dim g As Graphics = Graphics.FromImage(bmp)
        g.Clear(Color.Black)
        If (Visible = True) Then
            Dim pen As Pen = New Pen(Brushes.Yellow, 1)
            g.DrawLine(pen, New Point(0, grbCamera.Height / 2), New Point(grbCamera.Width, grbCamera.Height / 2))
            g.DrawLine(pen, New Point(grbCamera.Width / 2, 0), New Point(grbCamera.Width / 2, grbCamera.Height))
        End If
        grbCamera.BackgroundImage = bmp.Clone()
        bmp.Dispose()
        g.Dispose()
    ...
    Private Sub FDawLine_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'm_Cross.X = image.Width / 2 + m_Param.Variable.Graphic.OffsetX
        'm_Cross.Y = image.Height / 2 + m_Param.Variable.Graphic.OffsetY
        ...
        chkUseScale.Checked = m_Param.Variable.Laser.UseScale
        m_Param.Flags.System.ProductLoaded = False
        m_Laser = New CLaser(m_Param, m_Timer)
        If (My.Computer.Name.ToLower.IndexOf("alin") < 0 AndAlso m_Laser.SwRunTimeError = True) Then
            FMessageBox.Show(CChangeLanguage.GetString("請重啟電腦和雷射"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
            Environment.Exit(0)
        End If
        ...
        '    End
        'End If
        'Dim cylName(0 To 64) As String
        'Dim snrName(0 To 64) As String
        ...
        'cylName(6) = "WorkholderVaccum"
        'cylName(7) = "TrayInMagnet"
        ...
        cboCalibrationRange.Items.Add(eCalibrationSize.e4x4)
        ...
        Try
            ...
            Select Case e.Node.Level
                ...
                Case 1
                    ...
                    '        btnWorkholderMove0_Click(btnWorkholderMove1, Nothing)
                    '    ElseIf (m_SelectedStep = 2) Then
                    '        btnWorkholderMove0_Click(btnWorkholderMove2, Nothing)
                    '    End If
                    'End If
    ...
    Private Sub Control_MouseDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grbCamera.MouseDown
        If (e.Button = MouseButtons.Left AndAlso CLoginInformation.CurrentUser.Permission > 0) Then
            m_MouseX = e.X
            m_MouseY = e.Y
        ...
        End If
    End Sub
    'Private Sub ImageDisplay1_MouseDown(ByVal Location As System.Drawing.PointF, ByVal e As System.Windows.Forms.MouseEventArgs) 'Handles ImageDisplay1.ImageDisplay_MouseDown2
    '    If (e.Button = MouseButtons.Left AndAlso CLoginInformation.CurrentUser.Permission > 0) Then
    '        m_MouseX = Location.X
    '        m_MouseY = Location.Y
    '        m_OldMouse = Location
    ...
    '    End If
    'End Sub
    Private Sub Control_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grbCamera.MouseUp 'Handles ImageDisplay1.ImageDisplay_MouseUp2
    ...
    Private Sub Control_MouseMove(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs)
        'ImageDisplay1_ImageDisplay_MouseMove2(sender, e)
    End Sub

    'Private Sub ImageDisplay1_ImageDisplay_MouseMove2(ByVal Location As System.Drawing.PointF, ByVal e As System.Windows.Forms.MouseEventArgs)
    '    Try
    '        If m_Param.Flags.System.AutoRunProcedure = False Then
    '            m_MouseX = Location.X
    '            m_MouseY = Location.Y
    '            lblStatusX.Text = ((m_MouseX) * PX2MM).ToString("F3").PadLeft(6, " ")
    '            lblStatusY.Text = ((grbCamera.Height - m_MouseY) * PX2MM).ToString("F3").PadLeft(6, " ")
    '            lblXPx.Text = Location.X.ToString()
    '            lblYPx.Text = Location.Y.ToString()
    ...
    '                Exit Sub
    '            End If
    '            If (m_StartEdit AndAlso (m_SelectedNodes Is Nothing OrElse m_SelectedNodes.Count <= 1)) Then
    '                If (CLoginInformation.CurrentUser.Permission > 0) Then
    ...
    Public Function DrawSelectingLine() As Boolean
        ...
        Try
            ...
            If (chkTest.Checked = True) Then
                Call AddLog("切:" & m_LineSelected.ToString & "測試")
            Else
                Call AddLog("切:" & m_LineSelected.ToString)
            End If



            If (m_Param.Variable.Laser.UseScale = True) Then
            ...
            Else
                newX1InGalvo = m_LineSelected.StartX * m_Laser.CalibrationK
                newY1InGalvo = m_LineSelected.StartY * m_Laser.CalibrationK
                newX2InGalvo = m_LineSelected.EndX * m_Laser.CalibrationK
                newY2InGalvo = m_LineSelected.EndY * m_Laser.CalibrationK
            End If
            ...
            If (m_LineSelected IsNot Nothing AndAlso chkTest.Checked = True) Then
            ...
            If (m_LineSelected.RepeatMode = eRepeatMode.Mode1) Then
                For i = 1 To m_LineSelected.RepeatCount
                    Call m_Laser.JumpAbs(CInt(newX1InGalvo), CInt(newY1InGalvo))
                    Call m_Laser.MarkAbs(CInt(newX2InGalvo), CInt(newY2InGalvo))
                Next
            Else
                Call m_Laser.JumpAbs(CInt(newX1InGalvo), CInt(newY1InGalvo))
                For i = 1 To m_LineSelected.RepeatCount
                    If (i Mod 2 = 0) Then
                        Call m_Laser.MarkAbs(CInt(newX1InGalvo), CInt(newY1InGalvo))
                    Else
                        Call m_Laser.MarkAbs(CInt(newX2InGalvo), CInt(newY2InGalvo))
                    End If
                Next
    ...
    Private Sub DrawSelectingWorkStep()
        ...
        Try
            ...
            '    End If
            'Loop
            Call AddLog("切: 第" & m_SelectedStep.ToString() & "步完成！")
            AddSelectionLine()
        Catch ex As Exception
            FMessageBox.Show(Me, CChangeLanguage.GetString("剪切選擇線錯誤。"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, Windows.Forms.MessageBoxIcon.Error, True, ex)
        End Try
    ...
    Private m_IsCutLineOnly As Boolean
    Private Sub brnDraw_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDrawSelecting.Click
        ...
        If (btnDrawSelecting.Text = CChangeLanguage.GetString("停止")) Then
            ...
            Call AddLog("切: 第" & m_SelectedStep.ToString() & "步已取消！")
            AddSelectionLine()
        Else
            If (m_CurrentProject Is Nothing) Then
                FMessageBox.Show(Me, CChangeLanguage.GetString("請先打開項目"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, Windows.Forms.MessageBoxIcon.Error)
            ...
            '        Exit Do
            '    ElseIf cylStatus = enumCylSts.TimeOut Then
            '        Dim res As DialogResult = FMessageBox.Show(Me, CChangeLanguage.GetString("<平台解真空>動作時間過久異常!!") & vbCrLf & CChangeLanguage.GetString("您要繼續切割嗎？"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.YesNo, MessageBoxIcon.Error)
            ...
            '    End If
            'Loop
            'Call m_DA.DigO.CylGo(enumCyl.WorkholderStand, enumCyllogic.Action)
            'Do
            '    Call System.Windows.Forms.Application.DoEvents()
            '    Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderStand, cylStatus)
            '    If cylStatus = enumCylSts.Ready Then
            '        Exit Do
            '    ElseIf cylStatus = enumCylSts.TimeOut Then
            '        Dim res As DialogResult = FMessageBox.Show(Me, CChangeLanguage.GetString("<平台支架>動作時間過久異常!!") & vbCrLf & CChangeLanguage.GetString("您要繼續切割嗎？"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.YesNo, MessageBoxIcon.Error)
            '        If (res = Windows.Forms.DialogResult.No) Then
            '            btnDrawSelecting.Enabled = True
            '            Exit Sub
            '        End If
            ...
            Try
                ...
                If (m_LineSelected IsNot Nothing) Then
                    ...
                    Call AddLog("切:" & m_LineSelected.ToString() & "完成！")
                    AddSelectionLine()
                ElseIf (m_SelectedStep > 0) Then
                    If (chkTest.Checked = True) Then
                        Call AddLog("切: 第" & m_SelectedStep.ToString() & "步測試")
                    Else
                        Call AddLog("切: 第" & m_SelectedStep.ToString() & "步")
                    End If
    ...
    Private Function CheckRTCOutRange(ByVal pX As Integer, ByVal pY As Integer) As Boolean
        Try
            ...
            '    Return True
            'End If
            If (pX < -32768 OrElse pX > 32767) Then
                Return True
            End If
            If (pY < -32768 OrElse pY > 32767) Then
                Return True
            End If
        Catch ex As Exception
        End Try
    ...
    Private Sub btnRun_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRun.Click
        ...
        'Dim res As Integer = CheckLineOutRange()
        'If (res <> -1) Then
        '    FMessageBox.Show(Me, CChangeLanguage.GetString(m_ListLine(res).ToString()) & CChangeLanguage.GetString("輸出範圍"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '    Exit Sub
        'End If
        If (SortJob() = False) Then
            Me.Enabled = True
    ...
    Private Sub SystemInitial()
        Try
            ...
            m_SysProcessNum(enumSys.MainCtl) = 30
    ...
    Private Sub btnNewProject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNewProject.Click
        Try
            ...
            If (fnew.IsOK) Then
                ...
                Call AddLog("添加新項目：", m_CurrentProject.ProjectName)
                m_FlagSave = False
                m_StartEdit = False
    ...
    Private Sub btnOpenProject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOpenProject.Click
        Try
            ...
            If (fopenproject.IsOK) Then
                ...
                m_FLoading.Dispose()
                m_FLoading = Nothing
                Call AddLog("加載項目：", m_CurrentProject.ProjectName)
                PropertyGrid1.SelectedObject = Nothing
            End If
        Catch ex As Exception
            ...
            If (m_StartEdit = True) Then 'Save edit position
                For i = 0 To m_ListLine.Count - 1
                    If m_LineSelected.ID = m_ListLine(i).ID Then
                        m_ListLine(i) = m_LineSelected
                        Exit For
                    End If
                Next
                m_LineSelectedOld = m_LineSelected.Clone()
                Dim res As Boolean = MLineManager.SaveListLine(CProject.PROJECT_PATH & m_CurrentProject.ProjectName & "\Job\ListJob.job", m_ListLine, err)
                If (res = False) Then
    ...
    Private Sub btnGuideON_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuideON.Click
        m_Laser.LaserGuide = True
        Me.btnMainGuideOFF.BackgroundImage = Global.LaserCutting.My.Resources.Resources.pulselaser3
    End Sub
    ...
    Private Sub btnCorrection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCorrection.Click
        ...
        Try
            If (m_Corection.LaserOn) Then
                If (m_Corection.LoopDraw = True) Then
                    m_ButtonClick = 6
                Else
                    m_ButtonClick = 1
                End If
    ...
    '            m_LaserReticle.X = ImageDisplay1.ImageX / 2
    '            m_LaserReticle.Y = ImageDisplay1.ImageY / 2
    ...
    Private Sub EnableDrawSelectingMode(ByVal status As Boolean)
        ...
        Else
            ...
            tblEditTable.Enabled = Not status
    ...
    Private Sub TabControl1_Selecting(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TabControlCancelEventArgs) Handles TabControl1.Selecting
        If (m_Param.Flags.System.AutoRunProcedure = True OrElse m_StartEdit = True) Then
            e.Cancel = True
            Exit Sub
        End If
        ...
        Me.Text = CChangeLanguage.GetString("手動雷射切割機")
        Select Case e.TabPage.Name
            Case TabPageCorrection.Name
                ...
                If (TabControl2.SelectedTab Is TabPageIOStatus) Then
                    TimerGetRTC4IO.Enabled = True
                Else
                    TimerGetRTC4IO.Enabled = False
                End If
            ...
            Case TabPageAlignment.Name
                ...
                Do
                    Application.DoEvents()
                Loop Until m_LoadedMatchingCount = m_WantLoadMatchingCount
            Case TabPageLog.Name
                TimerGetRTC4IO.Enabled = False
                If (TabPageLog.Controls.Count = 0) Then
                    Dim f As FLog = New FLog()
                    f.TopLevel = False
                    f.TopMost = True
                    TabPageLog.Controls.Add(f)
                    f.Show()
                Else
                    TabPageLog.Controls.RemoveAt(0)
                    Dim f As FLog = New FLog()
                    f.TopLevel = False
                    f.TopMost = True
                    TabPageLog.Controls.Add(f)
            ...
            Case TabPagePassword.Name
                Dim x As Integer = 0
                Dim y As Integer = 0
                Dim tabIdx As Integer = -1
                Dim rect As Rectangle
                For i = 0 To TabControl1.TabPages.Count - 1
                    If (TabControl1.TabPages(i).Name <> TabPagePassword.Name) Then
                ...
                ContextMenuStripPassword.Size = New Size(rect.Width, ContextMenuStripPassword.Height)
                ContextMenuStripPassword.Show(TabControl1, New Point(x, y))
                e.Cancel = True
    ...
    Private Sub btnSaveCorrectionValue_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveCorrectionValue.Click
        Try
            ...
            m_CorrectionDataFilePath = m_Corection.SaveCorectionParam(True)
            ImagePointsInit()
        Catch ex As Exception
    ...
    Private Sub Movie()
        Try
            ...
            pImagePointsInit = ImagePointsInit()
    ...
    Private Sub UserInterfaceRunStatus(ByVal Status As Boolean)
        Try
            If Status Then '自動流程
                ...
                Call InvokeControlEnabled(False, tblEditTable)
            ...
            Else '停下來
                ...
                Call InvokeControlEnabled(True, tblEditTable)
    ...
    Private Sub Substrate_System_2_MoveOut()
        ...
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInForwardReverse, enumCyllogic.Normal)
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInRotateInverseReverse, enumCyllogic.Action)
        '            m_SysProcessNum(sysNum) = 1210
        '        Case 1210
        '            Call m_DA.DigO.CheckCylReady(enumCyl.TrayInUpDown, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        '                Call m_DA.DigO.CheckCylReady(enumCyl.TrayInForwardReverse, cylStatus)
        '                If cylStatus = enumCylSts.Ready Then
        '                    Call m_DA.DigO.CheckCylReady(enumCyl.TrayInRotateInverseReverse, cylStatus)
        '                    If cylStatus = enumCylSts.Ready Then
        '                        m_SysProcessNum(sysNum) = 1220
        '                    ElseIf cylStatus = enumCylSts.TimeOut Then
        '                        Call ShowErrorMessage(sysNum, "<收料選轉汽缸>動作時間過久異常!!")
        '                        m_SysProcessNum(sysNum) = 1215
        '                    End If
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    Call ShowErrorMessage(sysNum, "<收料汽缸後移>動作時間過久異常!!")
        '                    m_SysProcessNum(sysNum) = 1215
        '                End If
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        ...
        '            End If
        '        Case 1220
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInZUpDown, enumCyllogic.Action)
        '            m_SysProcessNum(sysNum) = 1230
        '        Case 1230
        '            Call m_DA.DigO.CheckCylReady(enumCyl.TrayInZUpDown, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        '                m_SysProcessNum(sysNum) = 1240
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料Z軸下降>動作時間過久異常!!")
        ...
        '            End If
        '        Case 1260
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInMagnet, enumCyllogic.Normal)
        '            m_SysProcessNum(sysNum) = 1270
        '        Case 1270
        '            Call m_DA.DigO.CheckCylReady(enumCyl.TrayInMagnet, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        ...
        '        Case 1280
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInZUpDown, enumCyllogic.Normal)
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInUpDown, enumCyllogic.Normal)
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInForwardReverse, enumCyllogic.Normal)
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInRotateInverseReverse, enumCyllogic.Normal)
        ...
        '                    End If
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    Call ShowErrorMessage(sysNum, "<收料汽缸上移>動作時間過久異常!!")
        '                    m_SysProcessNum(sysNum) = 1295
        '                End If
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料Z軸汽缸上移>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1295
        '            End If
        '        Case 1295
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        ...
        '        Case 1300
        '            m_Param.Flags.Pause.SysSubstrateFinished = True
        '            m_SysSts.SubstrateSys = enumStsSubstrateIDs.MoveOutOK : m_SysProcessNum(sysNum) = loop_COMMAND
        ...
        Try
            ...
            Select Case m_SysProcessNum(sysNum)
                ...
                Case 1200
                        ...
                        Else
                            If m_SysCmd.WorkholderSys = enumCmdWorkholderIDs.NoCommand Then
                                If Not m_SysSts.WorkholderSys = enumStsWorkholderIDs.Unknow Then
                                    ...
                                    m_SysProcessNum(sysNum) = 1220 '1210
                                End If
                            End If
                        End If
                    End If
                    ...
                    '                m_SysProcessNum(sysNum) = 1220
                    '            Case enumStsWorkholderIDs.SubstrateInError '往下跑

                    '        End Select
                    '    End If

                    'Case 1215
                    '    'If m_Timer.GetMilliseconds - m_SysTime(sysNum) >= m_Param.Variable.IO.DIDelay Then
                    '    m_SysTime(sysNum) = m_Timer.GetMilliseconds
                    '    Call m_Laser.DigitialOutSet(CLaser.PIN_OUTPUT, True)
                    '    m_SysProcessNum(sysNum) = 1217
                    '    'End If

                    'Case 1217
                    '    If m_Timer.GetMilliseconds - m_SysTime(sysNum) >= m_Param.Variable.IO.DODelay Then
                ...
                Case 1220
                    If m_Param.Flags.System.SysInitHomeOk = True Then
                        ...
                        If snrStatus = ACTIVE Then
                            'm_Param.Flags.System.Complete = True
                            m_SysTime(sysNum) = m_Timer.GetMilliseconds()
                            'm_Param.Idx.System.WorkStep = 0 ' m_Param.Idx.System.WorkStep + 1
                            'm_Param.Idx.System.JobIdx = 0
                    ...
                    If (m_Timer.GetMilliseconds() - m_SysTime(sysNum) >= m_Param.Variable.IO.DIDelay) Then
                        ...
                        If snrStatus = ACTIVE Then
                            ...
                            For i = 0 To m_ListLine.Count - 1
                                ...
                                'grbCamera.Controls(m_ListLine(i).ID).Visible = False
                                InvokeSelectedLine(False, CType(grbCamera.Controls(m_ListLine(i).ID), CLineCtrl))
                                'CType(grbCamera.Controls(m_ListLine(i).ID), CLineCtrl).IsSelect = False
                            Next
                        End If
                    End If
                ...
                Case 2000
                    ...
                    '        Select Case m_SysSts.WorkholderSys
                    '            Case enumStsWorkholderIDs.Unknow
                    '            Case enumStsWorkholderIDs.SubstrateOutOk
    ...
    Private Sub Workholder_System_1_Home()
        ...
        '                    m_SysProcessNum(sysNum) = 1220
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    Call ShowErrorMessage(sysNum, "<收料升降汽缸>動作時間過久異常!!")
        ...
        '                m_SysProcessNum(sysNum) = 1240
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料手臂伸出>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1212
        '            End If
        '        Case 1240
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, PseudoDevice.enumCyllogic.Normal)
        '            m_SysProcessNum(sysNum) = 1250
        '        Case 1250
        '            Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderForwardReverse, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        '                m_SysProcessNum(sysNum) = 1260
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<平台縮回>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1212
        '            End If
        '        Case 1260
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInZUpDown, enumCyllogic.Normal)
        ...
        '                m_SysProcessNum(sysNum) = 1280
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料Z升降汽缸>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1212
        '            End If
        ...
        Try
            ...
            Select Case m_SysProcessNum(sysNum)
                ...
                Case 1200
                    ...
                    '    If Status = enumMotionFlag.eSent Then m_SysProcessNum(sysNum) = 1206
                    'End If
                    'Case 1206
                    'Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderForwardReverse, cylStatus)
                    'If cylStatus = enumCylSts.Ready Then
                    '    m_SysProcessNum(sysNum) = 1208
                    'ElseIf cylStatus = enumCylSts.TimeOut Then
                    '    Call ShowErrorMessage(sysNum, "<平台收回>動作時間過久異常!!")
                    '    m_SysProcessNum(sysNum) = 1207
                    'End If
                    'Case 1207
                    'If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
                    '    Call LightNormalRun()
                    '    Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Normal)
                    '    m_SysProcessNum(sysNum) = 1206
                    'End If
                    'Case 1208
                    'axisID = enumAxis.WorkholderX
                    'If m_Motion.ChkStop(axisID) = enumMotionFlag.eReady Then
                    '    m_SysProcessNum(sysNum) = 1209
                    'End If
                    ' Case 1209
                    'm_SysProcessNum(sysNum) = 1210
                Case 1210
                ...
                Case 1220
                    ...
                    Try
                        ...
                        Dim angleInGalvo As Double = 0
                        Dim idx As Integer = m_lstLineOnWorkStep(m_Param.Idx.System.WorkStep)(m_Param.Idx.System.JobIdx)
                        Call AddLog("切:" & m_ListLine(idx).ToString)
                        ...
                        Else
                            newX1InGalvo = m_ListLine(idx).StartX * m_Laser.CalibrationK
                            newY1InGalvo = m_ListLine(idx).StartY * m_Laser.CalibrationK
                            newX2InGalvo = m_ListLine(idx).EndX * m_Laser.CalibrationK
                            newY2InGalvo = m_ListLine(idx).EndY * m_Laser.CalibrationK
                        End If
    ...
    Private Sub Workholder_System_5_SubstrateOut()
        ...
        '    Dim bStatus As enumMotionFlag
        '    Dim snrStatus As Boolean
        '    Dim snrStatusMotion As enumMotionFlag
        '    sysNum = enumSys.Workholder
        '    Select Case m_SysProcessNum(sysNum)
        '        Case 0, loop_COMMAND
        '        Case 1200
        ...
        '                If bStatus = enumMotionFlag.eSent Then m_SysProcessNum(sysNum) = 1230
        '            End If
        '        Case 1230
        '            'axisID = enumAxis.WorkholderX
        '            'If m_Motion.ChkStop(axisID) = enumMotionFlag.eReady Then
        '            '    Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Normal)
        '            '    m_SysProcessNum(sysNum) = 1237
        '            'End If
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Normal)
        '            m_SysProcessNum(sysNum) = 1237
        ...
        '                m_SysProcessNum(sysNum) = 1240
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<平台前移>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1238
        '            End If
        '        Case 1238
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        '                Call LightNormalRun()
        ...
        '                m_SysProcessNum(sysNum) = 1250
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料Z軸汽缸上升>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1245
        '            End If
        '        Case 1245
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        ...
        '            End If
        '        Case 1250
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInForwardReverse, enumCyllogic.Action)
        '            m_SysProcessNum(sysNum) = 1255
        '        Case 1255
        '            Call m_DA.DigO.CheckCylReady(enumCyl.TrayInForwardReverse, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        '                m_SysProcessNum(sysNum) = 1260
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料汽缸前移>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1258
        '            End If
        '        Case 1258
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        '                Call LightNormalRun()
        ...
        '                m_Motion.IsExternalParameters(axisID) = False
        '                Call m_Motion.MoveAbs(axisID, tarPos1, bStatus)
        '                If bStatus = enumMotionFlag.eSent Then m_SysProcessNum(sysNum) = 1270
        '            End If
        '        Case 1270
        ...
        '            Call m_DA.DigO.CheckCylReady(enumCyl.TrayInUpDown, cylStatus)
        '            If cylStatus = enumCylSts.Ready Then
        '                Call m_DA.DigO.CylGo(enumCyl.TrayInMagnet, enumCyllogic.Action)
        '                m_SysProcessNum(sysNum) = 1290
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料汽缸下降>動作時間過久異常!!")
        ...
        '                m_SysProcessNum(sysNum) = 1300
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<收料電磁鐵>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1295
        '            End If
        '        Case 1295
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        ...
        '            '    m_Motion.IsExternalParameters(axisID) = False
        '            '    Call m_Motion.MoveAbs(axisID, tarPos1, bStatus)
        '            '    If bStatus = enumMotionFlag.eSent Then m_SysProcessNum(sysNum) = 1350
        '            'End If
        '        Case 1350
        '            axisID = enumAxis.WorkholderX
        '            If m_Motion.ChkStop(axisID) = enumMotionFlag.eReady Then
        '                Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Action)
    ...
    Private Sub Workholder_System_2_SubstrateIn()
        ...
        '            '            m_SysProcessNum(sysNum) = 1217
        '            '        ElseIf cylStatus = enumCylSts.TimeOut Then
        '            '            Call ShowErrorMessage(sysNum, "<收料汽缸收回>動作時間過久異常!!")
        '            '            m_SysProcessNum(sysNum) = 1215
        '            '        End If
        '            '    ElseIf cylStatus = enumCylSts.TimeOut Then
        '            '        Call ShowErrorMessage(sysNum, "<收料汽缸上移>動作時間過久異常!!")
        ...
        '                m_Motion.IsExternalParameters(axisID) = False
        '                Call m_Motion.MoveAbs(axisID, tarPos1, bStatus)
        '                If bStatus = enumMotionFlag.eSent Then m_SysProcessNum(sysNum) = 1220
        '            End If
        '        Case 1220
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Action)
        '            'Call m_DA.DigO.CylGo(enumCyl.WorkholderVaccum, enumCyllogic.Action)
        ...
        '                m_SysProcessNum(sysNum) = 1240
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<平台汽缸前進>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1235
        '            End If
        '        Case 1235
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
        ...
        '                m_SysProcessNum(sysNum) = 1270
        '            ElseIf cylStatus = enumCylSts.TimeOut Then
        '                Call ShowErrorMessage(sysNum, "<平台支架>動作時間過久異常!!")
        '                m_SysProcessNum(sysNum) = 1265
        '            End If
        '        Case 1265
        '            If m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.OK Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Yes Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.No Or m_FErrorMsg(sysNum).Result = Windows.Forms.DialogResult.Cancel Then
    ...
    '                m_SysProcessNum(sysNum) = 1220
    '            Case 1215
    '                'If m_IsLeft = True Then
    '                m_SysProcessNum(sysNum) = 1220
    '                'Else
    '                'm_SysProcessNum(sysNum) = 2020
    '                'End If
    '            Case 1220
    ...
    '                'Case 2020
    '                '    Call m_MatchingToolRight.InputImage.CopyOf(m_SourceImage1st)
    '                '    Call m_MatchingToolRight.Run(False, EnumMatchShape.None)
    '                '    m_SysProcessNum(sysNum) = 2030
    '                'Case 2030
    '                '    Call Me.ImageDisplay1.StaticGraphics.Clear()
    '                '    If Not m_MatchingToolRight.IsRunning Then
    '                '        If m_MatchingToolRight.Results.Count > 0 Then
    '                '            Call Me.ImageDisplay1.StaticGraphics.Add(m_MatchingToolRight.Results.GetGraphics(CType(enumMatchingToolResultGraphicConstants.MatchRegion + enumMatchingToolResultGraphicConstants.Origin, enumMatchingToolResultGraphicConstants)))
    '                '            m_SysProcessNum(sysNum) = 2040
    '                '        Else
    '                '            m_SysProcessNum(sysNum) = 8020
    '                '        End If
    '                '    End If
    '                'Case 2040
    '                '    m_Param.Flags.Pause.SysWorkholderFinished = True
    '                '    m_SysSts.WorkholderSys = enumStsWorkholderIDs.ImageAlignmentOK
    '                '    m_Param.Pos.Workholder.ImageAlignT = m_MatchingToolRight.Results.Item(0).RotationD
    '                '    m_Param.Pos.Workholder.AlignRealX = (m_MatchingToolRight.Results.Item(0).TranslationX - m_SourceImage1st.Width / 2) * m_Param.Variable.Workholder.CCDRatioX
    '                '    m_Param.Pos.Workholder.AlignRealY = (m_MatchingToolRight.Results.Item(0).TranslationY - m_SourceImage1st.Height / 2) * m_Param.Variable.Workholder.CCDRatioY
    '                '    m_SysProcessNum(sysNum) = loop_COMMAND
    ...
    '                'Case 8020
    '                '    m_Param.Flags.Pause.SysWorkholderFinished = True
    '                '    m_SysSts.WorkholderSys = enumStsWorkholderIDs.ImageAlignmentErrorRight
    '                '    m_SysProcessNum(sysNum) = loop_COMMAND
    ...
    '                Call m_MatchingToolLeft.Pattern.SetRotaryRange(-30, 30)
    ...
    Public Sub EnableVision(ByVal status As Boolean, ByVal btn As Button)
        ...
        'TableLayoutPanel1.Enabled = status
        'PropertyGrid1.Enabled = status
        'btnDrawSelecting.Enabled = status
        'chkTest.Enabled = status
        ...
        Try
            If (m_Param IsNot Nothing) Then
                ...
                If (m_Param.Variable.Graphic.OffsetX <> numImageOffsetX.Value) Then
                    Call MLog.SaveLog("更改Image Offset X", m_Param.Variable.Graphic.OffsetX.ToString(), numImageOffsetX.Value.ToString(), logPath)
                    m_Param.Variable.Graphic.OffsetX = numImageOffsetX.Value
                    flag = True
                End If

                If (m_Param.Variable.Graphic.OffsetY <> numImageOffsetY.Value) Then
                    Call MLog.SaveLog("更改Image Offset Y", m_Param.Variable.Graphic.OffsetY.ToString(), numImageOffsetY.Value.ToString(), logPath)
                ...
                If (chkUseScale.Checked <> m_Param.Variable.Laser.UseScale) Then
                    Call MLog.SaveLog("更改 UseScale", m_Param.Variable.Laser.UseScale.ToString(), chkUseScale.Checked.ToString(), logPath)
                    m_Param.Variable.Laser.UseScale = chkUseScale.Checked
                    flag = True
                End If

                If (numDIDelay.Value <> m_Param.Variable.IO.DIDelay) Then
                    Call MLog.SaveLog("更改 DIDelay", m_Param.Variable.IO.DIDelay.ToString(), numDIDelay.Value.ToString(), logPath)
                    m_Param.Variable.IO.DIDelay = numDIDelay.Value
                    flag = True
                End If
                If (numDODelay.Value <> m_Param.Variable.IO.DODelay) Then
                    Call MLog.SaveLog("更改 DODelay", m_Param.Variable.IO.DODelay.ToString(), numDODelay.Value.ToString(), logPath)
    ...
    Private Sub ReloadCutLine(ByVal pWorkStep As Integer, ByVal pIdxLine As Integer)
        Try
            ...
            Else
                ...
                'Dim x1InImage As Double = m_ListOriginalLine(pIdxLine).StartXPX * ImageDisplay1.ImageX / grbCamera.Width
                'Dim y1InImage As Double = m_ListOriginalLine(pIdxLine).StartYPX * ImageDisplay1.ImageY / grbCamera.Height
                'Dim x2InImage As Double = m_ListOriginalLine(pIdxLine).EndXPX * ImageDisplay1.ImageX / grbCamera.Width
                'Dim y2InImage As Double = m_ListOriginalLine(pIdxLine).EndYPX * ImageDisplay1.ImageY / grbCamera.Height

                'Call NewPos(offsetXInImage, offsetYInImage, angleInImage, centerXInImage, centerYInImage, x1InImage, y1InImage, newX1InImage, newY1InImage)
                'Call NewPos(offsetXInImage, offsetYInImage, angleInImage, centerXInImage, centerYInImage, x2InImage, y2InImage, newX2InImage, newY2InImage)

                'm_ListLine(pIdxLine).StartXPX = newX1InImage * grbCamera.Width / ImageDisplay1.ImageX
                'm_ListLine(pIdxLine).StartYPX = newY1InImage * grbCamera.Width / ImageDisplay1.ImageX
                'm_ListLine(pIdxLine).EndXPX = newX2InImage * grbCamera.Width / ImageDisplay1.ImageX
                'm_ListLine(pIdxLine).EndYPX = newY2InImage * grbCamera.Width / ImageDisplay1.ImageX
    ...
    Private Sub ClearSelectionLine()
        If (Me.InvokeRequired) Then
            Me.Invoke(Sub() ClearSelectionLine())
        Else
            If (m_LineSelected IsNot Nothing) Then
                CType(grbCamera.Controls(m_LineSelected.ID), CLineCtrl).IsSelect = False
                grbCamera.Refresh()
            End If
        End If
    End Sub

    Private Sub AddSelectionLine()
        If (Me.InvokeRequired) Then
            Me.Invoke(Sub() AddSelectionLine())
        Else
            If (m_LineSelected IsNot Nothing) Then
                CType(grbCamera.Controls(m_LineSelected.ID), CLineCtrl).IsSelect = True
                grbCamera.Refresh()
                Dim node() As TreeNode = TreeView1.Nodes.Find(m_LineSelected.ID, True)
                If (node IsNot Nothing AndAlso node.Length > 0) Then
                    TreeView1_NodeMouseClick(TreeView1, New System.Windows.Forms.TreeNodeMouseClickEventArgs(node(0), Windows.Forms.MouseButtons.Left, 1, 0, 0))
    ...
    Private Sub LoginToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LoginToolStripMenuItem.Click
        If (LoginToolStripMenuItem.Text = CChangeLanguage.GetString("登錄")) Then
            Dim f As FLogin = New FLogin(True)
            f.ShowDialog()
            If (f.LoginOK) Then
                LoginToolStripMenuItem.Text = CChangeLanguage.GetString("登出")
                Call SetPermission()
    ...
    '            Call AddLog("更改第" & numWorkStep.Value & "步的視覺")
    '            m_VisionStart = True
    '        Else
    ...
    '                Call m_MatchingTool(numWorkStep.Value - 1).Pattern.SetRotaryRange(-30, 30)
    ...
    '            Me.Text = CChangeLanguage.GetString("分數") & ": " & (m_MatchingTool(numWorkStep.Value - 1).Results.Item(0).Score * 100) & " " & CChangeLanguage.GetString("橫向偏差量") & ": " & (offsetXInImage * PX2MM).ToString("F3") & "mm " & CChangeLanguage.GetString("縱向偏差量") & ": " & (offsetYInImage * PX2MM).ToString("F3") & "mm " & CChangeLanguage.GetString("角度") & ": " & angleInImage & " deg"
    '            Call Me.ImageDisplay1.StaticGraphics.Add(m_Cross)
    ...
    '            m_MatchingTool(pWorkStep).RunParams.SetRotaryRange(-30, 30)
    '            m_MatchingTool(pWorkStep).InputImage.CopyOf(img)
    ...
    '                m_Param.Pos.Workholder.ImageAlignT = 0 ' -m_MatchingTool(pWorkStep).Results.Item(0).RotationD
    ...
    Private Sub TimerGetRTC4IO_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TimerGetRTC4IO.Tick
        Try
            If (m_Laser IsNot Nothing) Then
                ...
                For i = 0 To 15
                    Dim ptbIn As PictureBoxEx = CType(TableLayoutPanel2.Controls.Find("ptbIn" & (15 - i), False)(0), PictureBoxEx)
                    Dim ptbOut As PictureBoxEx = CType(TableLayoutPanel2.Controls.Find("ptbOut" & (15 - i), False)(0), PictureBoxEx)
    ...
    Private Sub ptbOut_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ptbOut0.Click, ptbOut1.Click, ptbOut2.Click, ptbOut3.Click, ptbOut4.Click, ptbOut5.Click, ptbOut6.Click, ptbOut7.Click, ptbOut8.Click, ptbOut9.Click, ptbOut10.Click, ptbOut11.Click, ptbOut12.Click, ptbOut13.Click, ptbOut14.Click, ptbOut15.Click
        Try
            If (m_Laser IsNot Nothing) Then
                ...
                'Else
                '    ctrl.TurnOn = Not ctrl.TurnOn
                '    m_Laser.DigitialOutSet(pPin, ctrl.TurnOn)
                'End If
                ctrl.Enabled = True
            End If
        Catch ex As Exception
    ...
    Private Sub ptbOut15_MouseMove(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles ptbOut9.MouseMove, ptbOut8.MouseMove, ptbOut7.MouseMove, ptbOut6.MouseMove, ptbOut5.MouseMove, ptbOut4.MouseMove, ptbOut3.MouseMove, ptbOut2.MouseMove, ptbOut15.MouseMove, ptbOut14.MouseMove, ptbOut13.MouseMove, ptbOut12.MouseMove, ptbOut11.MouseMove, ptbOut10.MouseMove, ptbOut1.MouseMove, ptbOut0.MouseMove
        Dim ctrl As PictureBoxEx = CType(sender, PictureBoxEx)
    ...
    Public Function ImagePointsInit(Optional ByVal pPath As String = "D:\DataSettings\LaserMachine\Machine\Parameter") As Boolean
        Try
            ...
            For aryIdx As Integer = 0 To UBound(m_GridXInGalvoPose) Step 1
                ...
                m_GridXInGalvoPose(aryIdx) = Val(str(2))
                m_GridYInGalvoPose(aryIdx) = Val(str(3))
                ...
                dgvGalvoValue.Rows.Add(New Object() {aryIdx + 1, m_GridXInGalvoEncoder(aryIdx), m_GridYInGalvoEncoder(aryIdx), m_GridXInGalvoPose(aryIdx), m_GridYInGalvoPose(aryIdx)})
            Next
    ...
    Private Sub btnClear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClear.Click
        If (FMessageBox.Show(Me, CChangeLanguage.GetString("您確定要刪除數據嗎？"), CChangeLanguage.GetString("警告"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes) Then
            ...
            Try
                ...
                For aryIdx = 0 To count - 1
                    For aryIdx2 = 0 To count - 1
                        m_GridXInCamPose(idx) = StepX * (aryIdx + 1) + m_Param.Variable.Graphic.OffsetX
                        m_GridYInCamPose(idx) = grbCamera.Height - (StepY * (aryIdx2 + 1)) + m_Param.Variable.Graphic.OffsetY
                        dgvImageValue.Rows.Add(New Object() {idx + 1, m_GridXInCamPose(idx), m_GridYInCamPose(idx)})
                        idx += 1
                    Next
                Next
    ...
    Private Sub btnSaveCalibration_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveCalibration.Click
        ...
        Try
            ...
            For idx = 0 To strImage.Length - 1
                ...
                strGalvo(idx) = dgvGalvoValue.Rows(idx).Cells(1).Value.ToString() & "," & dgvGalvoValue.Rows(idx).Cells(2).Value.ToString() & "," & dgvGalvoValue.Rows(idx).Cells(3).Value.ToString() & "," & dgvGalvoValue.Rows(idx).Cells(4).Value.ToString()
            Next
    ...
    Private Sub btnWorkholderMove0_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnWorkholderMove0.Click, btnWorkholderMove1.Click, btnWorkholderMove2.Click, btnWorkholderMove3.Click
        ...
        '    Do
        '        Call System.Windows.Forms.Application.DoEvents()
        '        Call m_DA.DigO.CheckCylReady(enumCyl.TrayInUpDown, cylStatus1)
        '        If cylStatus1 = enumCylSts.Ready Then
        '            Exit Do
        '        ElseIf cylStatus1 = enumCylSts.TimeOut Then
        '            'Call MsgBox("<收料汽缸上升>動作時間過久異常!!", MsgBoxStyle.Information, "Information")
        ...
        '    Do
        '        Call System.Windows.Forms.Application.DoEvents()
        '        Call m_DA.DigO.CheckCylReady(enumCyl.TrayInForwardReverse, cylStatus2)
        '        If cylStatus2 = enumCylSts.Ready Then
        '            Exit Do
        '        ElseIf cylStatus2 = enumCylSts.TimeOut Then
        '            'Call MsgBox("<收料汽缸後移>動作時間過久異常!!", MsgBoxStyle.Information, "Information")
        '            FMessageBox.Show(Me, CChangeLanguage.GetString("<收料汽缸後移>動作時間過久異常!!"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '            Exit Try
        '        End If
        '    Loop
    ...
    Private Sub btnWorkholderSet0_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnWorkholderSet0.Click, btnWorkholderSet1.Click, btnWorkholderSet2.Click, btnWorkholderSet3.Click
        'Call UserInterfaceRunStatus2(False)
        'Try
        '    Dim idx As Integer
        '    Dim axisNum As enumAxis
    ...
    Private Sub btnCyl_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCylinder0.Click, btnCylinder1.Click, btnCylinder2.Click, btnCylinder3.Click, btnCylinder4.Click, btnCylinder5.Click, btnCylinder6.Click, btnCylinder7.Click, btnCylinder8.Click, btnCylinder9.Click, btnCylinder10.Click, btnCylinder11.Click, btnCylinder13.Click, btnCylinder12.Click, btnCylinder15.Click, btnCylinder14.Click
        'Call UserInterfaceRunStatus2(False)
        'Try
        '    Dim cylStatus As enumCylSts
        '    Dim idx As Integer
        ...
        '    Select Case idx
        '        Case 0
        '            btnCylinder0.BackColor = Color.DarkGray
        ...
        '                    Exit Do
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    'Call MsgBox("<收料汽缸下降>下降時間過久異常!!請調整下感應器位置!!", MsgBoxStyle.Information)
        '                    FMessageBox.Show(Me, CChangeLanguage.GetString("<收料汽缸下降>下降時間過久異常!!請調整下感應器位置!!"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '                    Exit Do
        '                End If
        '            Loop
        '        Case 1
        '            btnCylinder0.BackColor = Color.White
        '            btnCylinder1.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInUpDown, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.TrayInUpDown, cylStatus)
        ...
        '                    Exit Do
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    'Call MsgBox("<收料汽缸上升>上升時間過久異常!!請調整上感應器位置!!", MsgBoxStyle.Information)
        ...
        '                End If
        '            Loop
        '            btnCylinder2.BackColor = Color.DarkGray
        ...
        '                    Exit Do
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    'Call MsgBox("<取料汽缸前移>下降時間過久異常!!請調整下感應器位置!!", MsgBoxStyle.Information)
        '                    FMessageBox.Show(Me, CChangeLanguage.GetString("<收料汽缸上升>上升時間過久異常!!請調整上感應器位置!!"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '                    Exit Do
        '                End If
        '            Loop
        ...
        '            btnCylinder3.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInForwardReverse, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.TrayInForwardReverse, cylStatus)
        ...
        '                    Exit Do
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    'Call MsgBox("<取料汽缸後移>上升時間過久異常!!請調整上感應器位置!!", MsgBoxStyle.Information)
        '                    FMessageBox.Show(Me, CChangeLanguage.GetString("<取料汽缸後移>上升時間過久異常!!請調整上感應器位置!!"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '                    Exit Do
        '                End If
        '            Loop
        '        Case 4
        '            btnCylinder4.BackColor = Color.DarkGray
        ...
        '            btnCylinder5.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInMagnet, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.TrayInMagnet, cylStatus)
        ...
        '                End If
        '            Loop
        '            btnCylinder6.BackColor = Color.DarkGray
        ...
        '            btnCylinder7.BackColor = Color.DarkGray
        '            '慢速下降
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInZUpDown, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        ...
        '                End If
        '            Loop
        '            btnCylinder8.BackColor = Color.DarkGray
        ...
        '            btnCylinder9.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderForwardReverse, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderForwardReverse, cylStatus)
        ...
        '            Loop
        '        Case 10
        '            btnCylinder10.BackColor = Color.DarkGray
        ...
        '            btnCylinder11.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.TrayInRotateInverseReverse, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.TrayInRotateInverseReverse, cylStatus)
        ...
        '        Case 14
        '            btnCylinder14.BackColor = Color.DarkGray
        '            btnCylinder15.BackColor = Color.White
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderStand, enumCyllogic.Action)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderStand, cylStatus)
        ...
        '                    Exit Do
        '                ElseIf cylStatus = enumCylSts.TimeOut Then
        '                    FMessageBox.Show(Me, CChangeLanguage.GetString("<平台支架>動作時間過久異常!!"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
        '                    Exit Do
        '                End If
        '            Loop

        '        Case 15
        '            btnCylinder14.BackColor = Color.White
        '            btnCylinder15.BackColor = Color.DarkGray
        '            Call m_DA.DigO.CylGo(enumCyl.WorkholderStand, enumCyllogic.Normal)
        '            Do
        '                Call System.Windows.Forms.Application.DoEvents()
        '                Call m_DA.DigO.CheckCylReady(enumCyl.WorkholderStand, cylStatus)
        ...
        If (m_CorrectionDataFilePath <> "") Then
            Try
                ...
                If (Not System.IO.Directory.Exists("C:\RTC4 Files")) Then
                    FMessageBox.Show(Me, CChangeLanguage.GetString("文件夾 C:\RTC4 文件不存在"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
    ...
    Private Sub btnCalScale_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCalScale.Click
        numCalScale.Value = numLengthScaleMM.Value / numLengthScalePX.Value
    End Sub

    Private Sub btnSaveScale_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveScale.Click
        Try
            Dim logPath As String = "D:\DataSettings\LaserMachine\Machine\Parameter\Log"
            If (Not System.IO.Directory.Exists(logPath)) Then
                System.IO.Directory.CreateDirectory(logPath)
            End If
            logPath &= "\" & DateTime.Now.ToString("yyyyMMdd") & ".log"
            Call MLog.SaveLog("更改 Scale Pixel to mm", m_Param.Variable.Graphic.PX2MM.ToString("F16"), numPX2MM.Value.ToString("F16"), logPath)
            m_Param.Variable.Graphic.PX2MM = numCalScale.Value
            PX2MM = m_Param.Variable.Graphic.PX2MM
            numPX2MM.Value = PX2MM
            Call m_Param.WriteDataToFile("D:\DataSettings\LaserMachine\Machine\Parameter\Parameter.xml")
        Catch
        End Try
    End Sub

    Private Sub btnMarkLineScale_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMarkLineScale.Click
        btnMarkLineScale.Enabled = False
        Try

            If (chkScaleUseLaser.Checked = False) Then
                m_Laser.LaserEmission = False
                m_Laser.LaserGuide = True

            Else
                m_Laser.PRR = numScaleLaserPRR.Value.ToString()
                m_Laser.OperatingPower = numScaleLaserPower.Value.ToString()
                m_Laser.LaserGuide = False
                m_Laser.LaserEmission = True
            ...
            Dim idx As Integer = 0
            Dim Line As CLine = New CLine()
            Line.StartX = 0 - numLengthScaleMM.Value / 2
            Line.EndX = 0 + numLengthScaleMM.Value / 2
            Line.StartY = 0
            Line.EndY = 0

            CenterX = (Line.StartX + Line.EndX) / 2
            CenterY = (Line.StartY + Line.EndY) / 2

            Call NewPos(OffsetX, OffsetY, Angle, CenterX, CenterY, Line.StartX, Line.StartY, NewX1, NewY1)
            Call NewPos(OffsetX, OffsetY, Angle, CenterX, CenterY, Line.EndX, Line.EndY, NewX2, NewY2)

            If (CheckRTCOutRange(NewX1, NewY1) = True) Then
                FMessageBox.Show(Me, CChangeLanguage.GetString(Line.ToString()) & CChangeLanguage.GetString("輸出範圍"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
                Exit Sub
            End If
            If (CheckRTCOutRange(NewX2, NewY2) = True) Then
                FMessageBox.Show(Me, CChangeLanguage.GetString(Line.ToString()) & CChangeLanguage.GetString("輸出範圍"), CChangeLanguage.GetString("錯誤"), MessageBoxButtons.OK, MessageBoxIcon.Error)
            ...
            End If
            Call m_Laser.setstartlist(1)
            Call m_Laser.setScannerDelays(Line.JumpDelay, Line.MarkDelay, Line.PolygonDelay)
            Call m_Laser.SetLaserDelays(Line.LaserOnDelay, Line.LaserOffDelay)
            Call m_Laser.SetJumpSpeed(Line.JumpSpeed)
            Call m_Laser.SetMarkSpeed(Line.MarkSpeed)

            If (Line.RepeatMode = eRepeatMode.Mode1) Then
                For i = 1 To Line.RepeatCount
                    Call m_Laser.JumpAbs(CInt(NewX1 * 1000), CInt(NewY1 * 1000))
                    Call m_Laser.MarkAbs(CInt(NewX2 * 1000), CInt(NewY2 * 1000))
                Next
            Else
    ...
    Private Sub numPRR_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles numPRR.ValueChanged
        numScaleLaserPRR.Value = numPRR.Value
    End Sub
    ...
    Private Sub numPower_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles numPower.ValueChanged
        numScaleLaserPower.Value = numPower.Value
    End Sub
    ...
    Private Sub btnHome_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnHome.Click
        Try
            Call AddLog(CChangeLanguage.GetString("復歸位置"))
    ...
    Private Sub lnkLanguage_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkLanguage.LinkClicked
        ...
        Dim p As Point = pnEdit.Location
        Try
        ...
        pnEdit.Location = p
        If (m_Laser.LaserGuide = True) Then
            btnMainGuideOFF.Text = "X"
        Else
            btnMainGuideOFF.Text = ""
        End If
    End Sub
    ...
    'Private Sub numImageOffsetX_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles numImageOffsetX.ValueChanged, numImageOffsetY.ValueChanged
    '    Try

    '        m_Cross.X = VisionImageWidth / 2 + numImageOffsetX.Value
    '        m_Cross.Y = VisionImageHeight / 2 + numImageOffsetY.Value
    '        m_Cross.Length = 1000
    '        Me.ImageDisplay1.StaticGraphics.Clear()
    '        Call Me.ImageDisplay1.StaticGraphics.Add(m_Cross)
    '    Catch ex As Exception
    ...
    Private Sub btnMarkAxis_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMarkAxis.Click
        Try
            ...
            Dim ArrowLength As Integer = 300


            'X axis 
            Call m_Laser.JumpAbs(0, 0)
            Call m_Laser.MarkAbs(5000 * m_Corection.KFactor / 1000, 0) '-

            Call m_Laser.JumpAbs(4500 * m_Corection.KFactor / 1000, 300 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(5000 * m_Corection.KFactor / 1000, 0) '\
            ...
            'X character
            Call m_Laser.JumpAbs(5100 * m_Corection.KFactor / 1000, 300 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(5500 * m_Corection.KFactor / 1000, -300 * m_Corection.KFactor / 1000) '/
            ...
            'Y axis 
            Call m_Laser.JumpAbs(0, 0)
            Call m_Laser.MarkAbs(0, 5000 * m_Corection.KFactor / 1000) '-

            Call m_Laser.JumpAbs(-300 * m_Corection.KFactor / 1000, 4500 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(0, 5000 * m_Corection.KFactor / 1000) '/
            ...
            'Y character
            Call m_Laser.JumpAbs(-200 * m_Corection.KFactor / 1000, 5700 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(0, 5400 * m_Corection.KFactor / 1000) '\

            Call m_Laser.JumpAbs(200 * m_Corection.KFactor / 1000, 5700 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(0, 5400 * m_Corection.KFactor / 1000) '/

            Call m_Laser.JumpAbs(0, 5100 * m_Corection.KFactor / 1000)
            Call m_Laser.MarkAbs(0, 5400 * m_Corection.KFactor / 1000) '|
    ...
    Private Sub grbCamera_MouseMove(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grbCamera.MouseMove
        Try
            If m_Param.Flags.System.AutoRunProcedure = False Then
                m_MouseX = e.X
                m_MouseY = e.Y
                ...
                lblStatusX.Text = galvoPosX.ToString("F3").PadLeft(6, " ")
                lblStatusY.Text = galvoPosY.ToString("F3").PadLeft(6, " ")
                lblXPx.Text = e.X.ToString()
                lblYPx.Text = e.Y.ToString()
                'lblStatusX.Text = ((m_MouseX - X0) * PX2MM).ToString("F3").PadLeft(6, " ")
                'lblStatusY.Text = ((grbCamera.Height - m_MouseY - Y0) * PX2MM).ToString("F3").PadLeft(6, " ")
                'Dim newXGalvo, newYGalvo As Double
                'Call ImagePointsToGalvoPosition(m_MouseX, m_MouseY, newXGalvo, newYGalvo)
                ...
                If (m_StartEdit AndAlso (m_SelectedNodes Is Nothing OrElse m_SelectedNodes.Count <= 1)) Then
                    If (CLoginInformation.CurrentUser.Permission > 0) Then
                        If (Math.Abs((m_LineSelected.StartXPX) - m_MouseX) <= m_MouseDeviation AndAlso Math.Abs((m_LineSelected.StartYPX) - m_MouseY) <= m_MouseDeviation) Then
                            grbCamera.Cursor = Cursors.Cross
                        ElseIf (Math.Abs((m_LineSelected.EndXPX) - m_MouseX) <= m_MouseDeviation AndAlso Math.Abs((m_LineSelected.EndYPX) - m_MouseY) <= m_MouseDeviation) Then
                            grbCamera.Cursor = Cursors.Cross
                        End If
                    End If
    ...
    Private Sub btnKeyCorrectionGalvo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnKeyCorrectionGalvo.Click
    ...
    Private Sub btnKeyImageCalibration_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnKeyImageCalibration.Click
        ...
        fInput.ColumnX = 1
        fInput.ColumnY = 2
        fInput.DecimalPlace = 0
        fInput.ShowDialog()
        fInput.Close()
    End Sub

    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        m_AxisVisible = Not m_AxisVisible
        DrawAxis(m_AxisVisible)
    End Sub
    ...
    Private Sub btnMainGuideOFF_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMainGuideOFF.Click
        btnMainGuideOFF.Enabled = False
        If (m_Laser.LaserGuide = True) Then
            m_Laser.LaserGuide = False
            Me.btnMainGuideOFF.BackgroundImage = Global.LaserCutting.My.Resources.Resources.pulselaser2
        Else
            ...
            Me.btnMainGuideOFF.BackgroundImage = Global.LaserCutting.My.Resources.Resources.pulselaser3
        End If
        btnMainGuideOFF.Enabled = True
    End Sub

    Private Sub btnMainGuideOFF_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMainGuideOFF.MouseHover
        If (m_Laser.LaserGuide = True) Then
            ToolTip1.SetToolTip(btnMainGuideOFF, CChangeLanguage.GetString("關閉紅光"))
        Else
            ToolTip1.SetToolTip(btnMainGuideOFF, CChangeLanguage.GetString("打開紅光"))
        End If
    End Sub

    Private Sub btnAddLine_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddLine.MouseHover
        ToolTip1.SetToolTip(btnAddLine, CChangeLanguage.GetString("添加新的切割線"))
    End Sub

    Private Sub btnDelete_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.MouseHover
        ToolTip1.SetToolTip(btnDelete, CChangeLanguage.GetString("刪除當前切割線"))
    End Sub

    Private Sub Button3_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.MouseHover
        ToolTip1.SetToolTip(Button3, CChangeLanguage.GetString("顯示/隱藏相機十字"))
    End Sub

    Private Sub btnPickColor_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPickColor.MouseHover
        ToolTip1.SetToolTip(btnPickColor, CChangeLanguage.GetString("更改切割線顏色"))
    End Sub
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        Try
            ...
            sx = m_LineSelected.StartX
            sy = m_LineSelected.StartY
            ex = m_LineSelected.EndX
            ey = m_LineSelected.EndY

            m_LineSelected.StartX = ex
            m_LineSelected.StartY = ey
            m_LineSelected.EndX = sx
            m_LineSelected.EndY = sy
