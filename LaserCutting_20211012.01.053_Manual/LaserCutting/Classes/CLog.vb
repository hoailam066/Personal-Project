...
Public Class Clog
    ...
    ''' <remarks></remarks>
    Public Sub New(ByVal pLogString As String)
        m_DateAndTime = DateTime.Now
    ...
    ''' <remarks></remarks>
    Public Sub New(ByVal pLogString As String, ByVal pNewValue As String)
        m_DateAndTime = DateTime.Now
        m_LogString = pLogString
    ...
    ''' <param name="pNewValue">New value</param>
    ''' <remarks></remarks>
    Public Sub New(ByVal pParamName As String, ByVal pOldValue As String, ByVal pNewValue As String)
        m_DateAndTime = DateTime.Now
...
Public Module MLog
    ...
    Public Function SaveLog(ByVal pLogString As String, Optional ByVal pPath As String = "") As Boolean
        SaveLog = True
        Try
            If (pPath = "") Then
                pPath = CheckRunLogLocation() & DateTime.Now.ToString("yyyyMMdd") & ".log"
            ...
            Dim oldLog As New List(Of Clog)
            LoadLog(oldLog, pPath)
            oldLog.Add(New Clog(pLogString))
            If (Not File.Exists(pPath)) Then
                Dim sw As StreamWriter = New StreamWriter(pPath)
                sw.Close()
                sw.Dispose()
    ...
    Public Function SaveLog(ByVal pParameterName As String, ByVal pOldValue As String, ByVal pNewValue As String, Optional ByVal pPath As String = "", Optional ByRef errorString As String = "") As Boolean
        SaveLog = True
        Try
            If (pPath = "") Then
                pPath = CheckRunLogLocation() & DateTime.Now.ToString("yyyyMMdd") & ".log"
            ...
            Dim oldLog As New List(Of Clog)
            LoadLog(oldLog, pPath)
            oldLog.Add(New Clog(pParameterName, pOldValue, pNewValue))
            If (Not File.Exists(pPath)) Then
                Dim sw As StreamWriter = New StreamWriter(pPath)
                sw.Close()
                sw.Dispose()
