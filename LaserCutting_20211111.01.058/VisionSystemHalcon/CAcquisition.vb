                        ...
                        Case ("Basler").ToLower()
                            ...
                            Select Case m_CameraInfo.Model.ToLower()
                                Case ("scA640-70fm").ToLower()
                                    Call m_AcqFifoTool.SetFramegrabberParam("color_space", "gray")
                                    Call m_AcqFifoTool.SetFramegrabberParam("camera_type", "7:0:0")
                                    Call m_AcqFifoTool.SetFramegrabberParam("packet_size", 4096)
                                    Call m_AcqFifoTool.SetFramegrabberParam("shutter", 150)
                                    Call m_AcqFifoTool.SetFramegrabberParam("start_async_after_grab_async", "disable")
                                Case Else
                                    'bayer_pattern'
                                    'bits_per_channel'
                        ...
                        Case "FLIR".ToLower()
                            Select Case m_CameraInfo.Model.ToLower()
                                Case ("Blackfly S BFS-U3-04S2C").ToLower()
                                    ...
                                    'burst_count'
                                    'swap_bytes'
                                    Call m_AcqFifoTool.SetFramegrabberParam("start_async_after_grab_async", "enable") 'enable disable
                            End Select
                    End Select
                Case Else
                    name = "DirectShow"
        ...
        Try
            ...
            Select Case DCamInfo.Model.ToLower()
                ...
                Case Else
                    ...
                    image5 = image8.RotateImage(CDbl(DCamInfo.RotateImageDegree), "bilinear")
            End Select
...
#Region "Constructors && Destructors"
    Public Sub New(ByVal pindex As Integer)
        ...
        Try
            ...
            Dim cameraType As String = "1394IIDC"
            If (My.Computer.Name.ToLower.IndexOf("alin") >= 0 OrElse My.Computer.Name.ToUpper.IndexOf("DESKTOP-4OLNLEK") >= 0) Then
            ...
            If (m_CameraCnt = 0) Then
                ...
                If valueList.TupleLength() > 0 Then
                    ...
                    If m_CameraCnt > 0 Then
                        ...
                        For j As Integer = 0 To (m_CameraCnt - 1)
                            'If m_CameraList(j).Substring(0, 19) = "basler&sca1000-30fm" Then
                            camID = camID + 1
                            m_Camera(camID) = New CDigCamera(cameraType, m_CameraList(j), camID)
                            Call ReadDataFromFile()
                            'fileExists = My.Computer.FileSystem.FileExists("D:\DataSettings\LaserSolder\HardWare\LutTrans" & camID & ".txt")
                            'If fileExists Then
                            '    strData = System.IO.File.ReadAllLines("D:\DataSettings\LaserSolder\HardWare\LutTrans" & camID & ".txt", System.Text.Encoding.Default)
                            '    strData = Split(strData(0), ",")
                            '    For aryIdx As Integer = 0 To 255
                            '        m_Camera(camID).LutAry(aryIdx) = CInt(strData(aryIdx))
                            '    Next aryIdx
                            'Else
                            '    For aryIdx As Integer = 0 To 255
                            '        m_Camera(camID).LutAry(aryIdx) = aryIdx
                            '    Next aryIdx
                            'End If
                            'End If
                        Next j
                    ...
                    If m_CameraCnt > 0 Then
                        ...
                        'ReDim mXmlExposure(mCamerasCount - 1)
                        'ReDim mXmlBrightness(mCamerasCount - 1)
                        'ReDim mXmlContrast(mCamerasCount - 1)
                    Else
                        Erase m_Camera
                    End If
                End If

                'name = Nothing : name = New HTuple("GigEVision")
                'query = Nothing : query = New HTuple("device")
                'Call HOperatorSet.InfoFramegrabber(name, query, information, valueList)
                'If valueList.TupleLength() > 0 Then
                '    For i = 0 To (valueList.TupleLength() - 1)
                '        Call m_CameraList.Insert(i, valueList.SArr(i))
                '    Next i
                '    m_CameraCnt = m_CameraList.Count
                '    If m_CameraCnt > 0 Then
                '        ReDim m_Camera(m_CameraCnt - 1)
                ...
                '            'If m_CameraList(j).Substring(0, 19) = "basler&sca1000-30fm" Then
                '            camID = camID + 1
                '            m_Camera(camID) = New CDigCamera("1394IIDC", m_CameraList(j), camID)
                '            'End If
                '        Next j
                '    End If
                '    m_CameraCnt = camID + 1
                ...
                '            'If m_CameraList(j).Substring(0, 19) = "basler&sca1000-30fm" Then
                '            camID = camID + 1
                '            m_Camera(camID) = New CDigCamera("GigEVision", m_CameraList(j), camID)
                '            'End If
                '        Next j
                '    End If
                '    m_CameraCnt = camID + 1
