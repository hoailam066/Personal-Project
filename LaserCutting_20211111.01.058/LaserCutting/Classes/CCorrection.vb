...
Public Class CCorrection
    ...
    Public Property Range As eCorrectionRange
        ...
        Set(ByVal value As eCorrectionRange)
            ...
            Else
                Dim x = m_Range
                m_Range = value
                m_ToTalSize = m_Size * (m_Range - 1) * m_KFactor / 1000000
                If (x <> m_Radius) Then
                    LoadCorrectionData()
                End If
            End If
    ...
    <Category("Common Parameters")>
    <DisplayName("Total size (mm)")>
    <Description("Size of the resulting figure")>
    <[ReadOnly](True)>
    ...
    Public Sub SaveCorectionParam2()
        ...
        End Try
    End Sub
    Public Function SaveCorectionParam() As String
        Try
            Dim path As String = "D:\DataSettings\LaserMachine\MachineData\User\"
            If (Directory.Exists(path) = False) Then
                Directory.CreateDirectory(path)
